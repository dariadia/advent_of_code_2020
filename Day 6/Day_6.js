...
const answersDataRaw = `wzaopvscxknyjtiul
belforsgdhuvmwc
...
const splitOnNewLine = (array) => {
  return array.split("\n")
}

const splitReplaceOnNewLine = (array) => {
...
const answersData = filterEmptyValues(splitLinesOnBreak(answersDataRaw))

let questionsTotal = 0
let questionsEveryYesTotal = 0

for (const group of answersData) {
  const sortedAnswers = splitReplaceOnNewLine(group)
  const uniques = new Set([...sortedAnswers])
  questionsTotal += uniques.size

  questionsEveryYesTotal += [...uniques].filter((character) =>
    filterEmptyValues(splitOnNewLine(group)).every((form) =>
      form.includes(character)
    )
  ).length
}

console.log(
  `The total count of the number of questions to which anyone answered "yes" is ${questionsTotal}`
)
...
console.log(
  `The total count of questions to which everyone answered "yes" is ${questionsEveryYesTotal}`
)
