...
const convertData = ( dataString ) => {
  const { groups } = /^(?<from>\d+)-(?<to>\d+) (?<character>.): (?<password>.*)$/.exec(dataString);
  const { from, to, character, password } = groups
  return { from, to, character, password }
}

const splitRawData = ( dataListRaw ) => {
  const dataArray = filterEmptyValues(dataListRaw.split(/\n/))
  const resultingArray = [];
  for (let item of dataArray) {
    let itemValues = filterEmptyValues(item.split(" ")).join(" ")
    let humanReadableData = convertData(itemValues)
    resultingArray.push(humanReadableData)
  }
  return resultingArray
}

export const passwordsList = splitRawData(passwordsListRaw)
